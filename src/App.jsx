...
async function fetchPrice(ticker) {
  try {
    const res = await fetch(`${WORKER_BASE}/quote?ticker=${encodeURIComponent(ticker)}`);
    ...
    return {
      ...
      // Support both array-of-objects and array-of-numbers from worker
      chartData: normaliseChartData(data.chartData),
...
function normaliseChartData(raw) {
  ...
  // Already [{price, time}]
  if (typeof raw[0] === "object" && raw[0] !== null && "price" in raw[0]) return raw;
  // Plain number array → wrap
  if (typeof raw[0] === "number") return raw.map((p, i) => ({ price: p, time: i }));
  return [];
}
...
async function searchStocksRemote(query) {
  ...
  return list.map(item => {
    if (typeof item === "string") {
      const ticker = item.endsWith(".AX") ? item : `${item}.AX`;
      return { ticker, name: item.replace(".AX", ""), sector: "Unknown" };
    }
    return {
      ticker: item.yahooTicker || item.ticker || (item.asxCode ? `${item.asxCode}.AX` : ""),
      name:   item.name || item.companyName || item.company || item.asxCode || "Unknown",
...
function Sparkline({ data, color }) {
  ...
  const points = data.map((d, i) =>
    `${(i / (data.length - 1)) * W},${H - ((d.price - min) / range) * H}`
  ).join(" ");
  return (
...
const S = {
  ...
  navBtn:    a => ({ background: a ? "#00d4ff15" : "transparent", color: a ? "#00d4ff" : "#4a6fa5", border: a ? "1px solid #00d4ff30" : "1px solid transparent", padding: "6px 16px", borderRadius: 4, cursor: "pointer", fontSize: "11px", letterSpacing: "1px", textTransform: "uppercase", fontFamily: "inherit" }),
  ...
  up:        { color: "#00ff88" },
  down:      { color: "#ff4d6d" },
  badge:     s => ({ fontSize: "9px", padding: "2px 6px", borderRadius: 3, background: s === "up" ? "#00ff8815" : s === "down" ? "#ff4d6d15" : "#94a3b815", color: s === "up" ? "#00ff88" : s === "down" ? "#ff4d6d" : "#94a3b8", border: `1px solid ${s === "up" ? "#00ff8830" : s === "down" ? "#ff4d6d30" : "#94a3b830"}` }),
  ...
  dot:       s => ({ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: s === "Underway" ? "#00ff88" : "#f59e0b", marginRight: 6 }),
};
...
export default function App() {
  ...
  const [selectedTicker, setSelectedTicker] = useState(null);   // FIX: store ticker, not object
  ...
  // Stable watchlist definition (name/sector metadata)
  const watchlistRef = useRef(
    DEFAULT_TICKERS.map(t => ({ ticker: t, name: t.replace(".AX", ""), sector: "Unknown" }))
  );
  const searchReqIdRef  = useRef(0);
  const searchTimerRef  = useRef(null);

  // Derived: selected stock object from stocks array
  const selectedStock = stocks.find(s => s.ticker === selectedTicker) ?? stocks[0] ?? null;

  // ── Load / refresh all stocks ────────────────────────────────────────────────
  ...
  // Keep newAlert ticker valid when watchlist changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    alerts.forEach(alert => {
      ...
      const hit =
        (alert.condition === "above" && stock.price >= alert.price) ||
  ...
  const handleSearch = useCallback((query) => {
    ...
    searchTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // Filter out already-watchlisted tickers
        const currentTickers = new Set(watchlistRef.current.map(s => s.ticker));
        setSearchResults(results.filter(s => !currentTickers.has(s.ticker)).slice(0, 6));
      } catch (err) {
        if (reqId !== searchReqIdRef.current) return;
        setSearchResults([]);
  ...
  const removeStock = useCallback((ticker) => {
    ...
    setStocks(prev => {
      ...
      // If removed stock was selected, fall back to first
  ...
  const dismissTriggered  = useCallback(idx => setTriggeredAlerts(prev => prev.filter((_, i) => i !== idx)), []);

  const priceColor = s => (s.change >= 0 ? "#00ff88" : "#ff4d6d");
    ...
    <div style={S.app}>
      ...
      <div style={S.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {loading
            ...
            : <span style={{ color: "#4a6fa5", fontSize: "11px" }}><span style={S.pulse} />~15min delay · {lastUpdated?.toLocaleTimeString()}</span>
          }
      ...
      <div style={S.main} className="main-pad">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} className="grid2">
            ...
            <div style={S.card} className="card-inner">
              ...
              <div style={{ position: "relative", marginBottom: 12 }}>
                <input
                  style={{ ...S.input, width: "100%", paddingLeft: 28 }}
                ...
                {searching && <div style={{ color: "#f59e0b", fontSize: 11, padding: "6px 0" }}>⟳ Searching ASX stocks…</div>}
                {searchError && <div style={{ color: "#ff4d6d", fontSize: 11, padding: "6px 0" }}>{searchError}</div>}
                {!searching && !searchError && searchQuery.trim() && searchResults.length === 0 && (
                  <div style={{ color: "#4a6fa5", fontSize: 11, padding: "6px 0" }}>No matching stocks found.</div>
                )}
                {fetchingTicker && <div style={{ color: "#f59e0b", fontSize: 11, padding: "6px 0" }}>⟳ Adding {fetchingTicker}…</div>}
              ...
              {loading
                ...
                : stocks.map(s => (
                    <div key={s.ticker} style={S.row(selectedStock?.ticker === s.ticker)} onClick={() => setSelectedTicker(s.ticker)}>
                      ...
                      <button
                        onClick={e => { e.stopPropagation(); removeStock(s.ticker); }}
                  ...
                  ? (
                    ...
                    <>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        {[["Sector", selectedStock.sector], ["Volume", selectedStock.volume ? `${(selectedStock.volume / 1e6).toFixed(1)}M` : "N/A"], ["Exchange", "ASX"], ["Currency", "AUD"]].map(([label, val]) => (
                          <div key={label} style={{ background: "#050d1a", borderRadius: 6, padding: "8px 12px" }}>
                            <div style={{ color: "#4a6fa5", fontSize: 10, marginBottom: 2 }}>{label}</div>
        ...
        {tab === "charts" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} className="grid2">
            {loading
              ? <div style={{ ...S.card, color: "#4a6fa5" }}>⟳ {loadingStatus}</div>
              : stocks.map(s => (
              ...
              <div key={n.id} style={{ padding: "14px 0", borderBottom: "1px solid #0f2d4a", display: "flex", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <span style={{ ...S.badge("neutral"), background: "#00d4ff10", color: "#00d4ff", border: "1px solid #00d4ff20" }}>{n.tag}</span>
            ...
            <div style={S.card} className="card-inner">
              ...
              {SHIPS.map(ship => (
                <div key={ship.id} style={S.shipRow}>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: "#e2e8f0", fontWeight: 600, marginBottom: 3 }}><span style={S.dot(ship.status)} />{ship.name}</div>
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <input
                    ...
                    onChange={e => setNewAlert(p => ({ ...p, price: e.target.value }))}
                  />
